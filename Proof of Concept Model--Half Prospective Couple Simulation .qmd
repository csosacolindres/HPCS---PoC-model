...
pFemales$PIN<-sample(nrow(pMales):(nrow(pMales)+nrow(pFemales)),nrow(pFemales),replace=F) + 1
...
#Give each agent a unique PIN
sMales$PIN<-sample(1:nrow(sMales),nrow(sMales),replace=F) + (popsize*2) + 1 
sFemales$PIN<-sample(nrow(sMales):(nrow(sMales)+nrow(sFemales)),nrow(sFemales),replace=F) + (popsize*2) + 2
  ...
  if (mca == "atm") {
    ...
    while (endcheck == 0) {
      ...
      for (u in 1:length(mdate)) {
        ...
        if (foffers[u] == T &
            matts[u] > mthresholds$thresh[mpos[u]]) {
          ...
          mthresholds$thresh[mpos[u]] <-
            ...
            .05 * (matts[u] - mthresholds$thresh[mpos[u]])
                   ...
                   mthresholds$thresh[mpos[u]] > matts[u]) {
      ...
      for (u in 1:length(fdate)) {
        ...
        if (moffers[u] == T &
            fatts[u] > fthresholds$thresh[fpos[u]]) {
          ...
          fthresholds$thresh[fpos[u]] <-
            ...
            .05 * (fatts[u] - fthresholds$thresh[fpos[u]])
                   ...
                   fthresholds$thresh[fpos[u]] > fatts[u]) {
      ...
      mutoffers <- moffers & foffers & matts >
        mthresholds$mateatt[mpos] &
        fatts > fthresholds$mateatt[fpos]
      ...
      for (u in 1:length(mutoffers)) {
        ...
        if (mutoffers[u] == T) {
          ...
          fthresholds$matepin[fthresholds$PIN == mthresholds$matepin[mpos[u]]] <-
            0
          mthresholds$matepin[mthresholds$PIN == fthresholds$matepin[fpos[u]]] <-
            0
      ...
      mthresholds$dates <- ifelse(
        mthresholds$matepin == 0,
        0,
        ifelse(mthresholds$matepin == mstartmate,
               mthresholds$dates + 1, 1)
      )
      
      fthresholds$dates <- ifelse(
        fthresholds$matepin == 0,
        0,
      ...
      same <-
        ifelse(sum(mthresholds$matepin != mstartmate) == 0, same + 1, 0)
      ...
      if (sum(mthresholds$avail) == 0 |
          sum(fthresholds$avail) == 0) {
        endcheck <- 1
      }
    ...
    females$modelguess <- fthresholds$matepin
    
    #Do the same for males
    males$modelguess <- mthresholds$matepin
    
    
    data2 <- rbind(females, males)
    data2$modelguess[data2$modelguess == 0] <- NA
...
ppAtm <-matechoice(pFemales, pMales, mca = "atm")
...
scAtm1 <-matechoice(pFemales, sMales, "atm")
scAtm2 <-matechoice(sFemales, pMales, "atm")
scAtm <-rbind(scAtm1, scAtm2)
...
# to compare : dist(rbind(ppGSA ))... SEE PIC

# need to compare this to null distribution : SEE PIC (this goes in  cbind(...) )
# do this 10000 times and compare actual difference ^^ to what you see here


# euc dist between primary and secondary partners 
matesDistGsaGsa <- sapply (ppGsa$PIN, function(x) 
  dist(rbind(ppGsa[ppGsa$PIN==ppGsa$modelguess[ppGsa$PIN==x], 18:33], 
             scGsa[scGsa$PIN==scGsa$modelguess[scGsa$PIN==x], 18:33])))

# Comapre to null distribution 
matesDistGsaGsaNull <- sapply( 1:10000, function(y) 
  mean(sapply (ppGsa$PIN, function(x) 
  dist(rbind(ppGsa[ppGsa$PIN==sample(ppGsa$PIN[ppGsa$sex != ppGsa$sex[ppGsa$PIN == x]], 1), 18:33], 
             scGsa[scGsa$PIN==scGsa$modelguess[scGsa$PIN==x], 18:33])))))


pGsa <- mean(mean(matesDistGsaGsa)>= matesDistGsaGsaNull)


# Can run ttest GSA to other model comparisons 
```
